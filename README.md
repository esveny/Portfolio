...
- Google reCAPTCHA v3 (contact anti-spam)
...
- `RECAPTCHA_V3_THRESHOLD` (optional, defaults to `0.5`)
...
- Client loads reCAPTCHA v3 and requests a token on submit.
- Form posts to `POST /api/contact`.
- API route validates the payload, verifies reCAPTCHA score, and inserts via service-role Supabase client.
